...
case class Felsenstein( model: JC69 ) {
  ...
  def DNA2Int(nuc: DNA): Int = nuc match {
    case A => 0
    case C => 1
    case G => 2
    case T => 3
  ...
  def likelihood(tree: Tree[DNA]): Array[Double] = tree match {
    case Branch(timeL, left, timeR, right) =>
      ...
      for (old <- List(A, C, G, T)) {
        ...
        for (nuc1 <- List(A, C, G, T); nuc2 <- List(A, C, G, T)) {
          println( model.likelihood(old, nuc1, timeL), " ",  distL(DNA2Int(nuc1)), " ",  model.likelihood(old, nuc2, timeR)," ",  distR(DNA2Int(nuc2)))
          dist(i) += model.likelihood(old, nuc1, timeL) * distL(DNA2Int(nuc1)) * model.likelihood(old, nuc2, timeR) * distR(DNA2Int(nuc2))
        }
      }
      dist
